...
void TFT_eSPI::init(SPI_TypeDef* SPIx)
{
	this->spi_init(SPIx);
	...
	this->commandList(ST7789_Init_List);
...
***************************************************************************************/
void TFT_eSPI::spi_init(SPI_TypeDef* SPIx)
{
	...
	if (SPIx == SPI1) {
		...
		pinMode(GPIOA, GPIO_Pin_5 | GPIO_Pin_7, GPIO_Mode_AF_PP);
		pinSet(GPIOA, GPIO_Pin_5 | GPIO_Pin_7, Bit_SET);
	}
	else if (SPIx == SPI2) {
		this->SPI = SPI2;
		RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE);
		pinMode(GPIOB, GPIO_Pin_13 | GPIO_Pin_15, GPIO_Mode_AF_PP);
		pinSet(GPIOB, GPIO_Pin_13 | GPIO_Pin_15, Bit_SET);
	}
...
{
	pinSet(RST.GPIO, RST.Pin, Bit_RESET);
	delay_ms(150);
	pinSet(RST.GPIO, RST.Pin, Bit_SET);
	delay_ms(100);
}
...
void TFT_eSPI::setRotation(uint8_t r)
{
	...
	case 0:
#ifdef CGRAM_OFFSET
		if (_init_width == 135) {colstart = 52; rowstart = 40; }
		else {colstart = 0; rowstart = 0; }
#endif
	...
	case 1:
#ifdef CGRAM_OFFSET
		if (_init_width == 135) {colstart = 40; rowstart = 53; }
		else {colstart = 0; rowstart = 0; }
#endif
	...
	case 2:
#ifdef CGRAM_OFFSET
		if (_init_width == 135) {colstart = 53; rowstart = 40; }
		else {colstart = 0; rowstart = 80; }
#endif
	...
	case 3:
#ifdef CGRAM_OFFSET
		if (_init_width == 135) {colstart = 40; rowstart = 52; }
...
void TFT_eSPI::pushPixels(const void* data_in, uint32_t len) {
	...
	if (_swapBytes) while (len--) {writeData_16(*data); data++; }
	else while (len--) {writeData_16S(*data); data++; }
}
